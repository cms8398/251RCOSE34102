...
{
    SchedulerType type = pre_sjf;
    qsort(processes, num_processes, sizeof(Process), compare_process_by_arrival);
    CPU cpu;

    MinHeap *rq =  create_heap(num_processes,compare_process_by_remaining_time);
    create_cpu(&cpu);
    ...
    printf("preemptive SJF Scheduling initiated...\n");
    ...
    { 
        ...
        {
            int result = compare_process_by_remaining_time(cpu_get_current(&cpu), rq->data[0]);
            if(result > 0)
            {
